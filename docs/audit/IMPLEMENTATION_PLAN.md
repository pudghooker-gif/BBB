...
- Keep deployment templates and runbook current; validate Nginx, PHP-FPM, systemd, cron, WebSocket origin/session-cookie/heartbeat controls, backup, restore, migration rehearsal, healthcheck, smoke/load verification, release evidence package checks, and rollback on a staging host.
...
- Verify queues, failed-job handling drills, scheduler-dispatched B2B jobs, WebSocket proxy, sandbox wallet flow, reports, settlements, backup/restore drills, smoke tests, measured smoke-load results, and a target-environment `release-evidence.json` package that passes `b2b:evidence-check --production`.
