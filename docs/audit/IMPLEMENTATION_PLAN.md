...
- Extend provider-specific status/rollback contracts beyond the current operator `transaction_status` lookup and bounded rollback recovery, then add authenticated step-up UI for manual actions and settlement-grade reconciliation reports.
