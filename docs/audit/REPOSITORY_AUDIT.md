...
- `php -v`: PHP 7.4.33.
- `composer validate --strict`: passes after adding proprietary license metadata; unused `laravel/ui`, legacy `fideloper/proxy`, unused SMS.to Laravel wrapper, dev-only debugbar, legacy `laracasts/presenter`, unused legacy factory, and legacy settings package direct dependencies were removed during dependency-trim work.
