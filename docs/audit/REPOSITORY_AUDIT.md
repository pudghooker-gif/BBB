...
- Admin B2B backoffice and operator portal are not implemented as full production workflows. A B2B-RBAC-protected read-only operations dashboard now boots inside the authenticated backend at `/backend/b2b`, and a signed tenant-scoped read-only operator portal is available at `/api/b2b/v1/portal` with JSON overview data at `/api/b2b/v1/portal/overview`. Session-bound web step-up routes/middleware are available for future dangerous B2B actions. Unified operator audit events now cover credential lifecycle actions, settlement export/submission/approval, denied privileged actions, and manual wallet actions.
