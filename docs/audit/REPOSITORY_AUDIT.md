...
- `php -v`: PHP 7.4.33.
- `composer validate --strict`: passes after adding proprietary license metadata; unused `laravel/ui`, legacy `fideloper/proxy`, unused SMS.to Laravel wrapper, dev-only debugbar, legacy `laracasts/presenter`, and unused legacy factory direct dependencies were removed during dependency-trim work.
...
- Remaining Laravel-major dependency blockers are actively used and need planned replacements or upstream upgrades: `anlutro/l4-settings` backs the `settings()` helper and backend `Settings::set/save()` writes; `jeremykenedy/laravel-roles` and transitive `eklundkristoffer/seedster` back role checks/seeders; `laravelcollective/html` powers many backend Blade forms; `proengsoft/laravel-jsvalidation` powers `JsValidator::formRequest(...)` in auth/user forms; `tymon/jwt-auth` backs API JWT integration; and `yajra/laravel-datatables-oracle` backs backend DataTables integration.
