...
- Admin B2B backoffice and operator portal are not implemented as full production workflows. A B2B-RBAC-protected operations dashboard now boots inside the authenticated backend at `/backend/b2b`, step-up protected credential lifecycle, manual wallet, and settlement workflow screens are available at `/backend/b2b/credentials`, `/backend/b2b/wallet/manual-actions`, and `/backend/b2b/settlements`, and a signed tenant-scoped read-only operator portal is available at `/api/b2b/v1/portal` with JSON overview data at `/api/b2b/v1/portal/overview`. Unified operator audit events now cover credential lifecycle actions, settlement export/submission/approval, denied privileged actions, and manual wallet actions.
