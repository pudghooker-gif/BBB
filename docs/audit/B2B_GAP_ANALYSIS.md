...
- Production-grade wallet state machine still needs provider-specific status contracts/certification, authenticated web step-up for manual actions, settlement-grade reconciliation reports, and full rollback-required recovery flows. The current foundation has explicit transition validation, append-only transition logging, recursive sensitive-field redaction, retryable `unknown`, retry-budget `dead_letter`, operator-scoped status lookup, reconciliation item scanning with conservative operator `transaction_status` resolution, privileged CLI guard, and audited CLI manual actions.
