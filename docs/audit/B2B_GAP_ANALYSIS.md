...
- B2B backend operations dashboard is wired into the existing authenticated backend at `/backend/b2b`; web step-up confirmation routes are available under `/backend/b2b/step-up/{action}`; B2B-RBAC and web-step-up protected manual wallet, settlement workflow, and credential lifecycle screens are available at `/backend/b2b/wallet/manual-actions`, `/backend/b2b/settlements`, and `/backend/b2b/credentials`.
...
- Dedicated B2B admin backoffice and full operator portal UX remain incomplete. A B2B-RBAC-protected backend operations dashboard now boots at `/backend/b2b`, step-up protected manual wallet, settlement workflow, and credential lifecycle screens are available at `/backend/b2b/wallet/manual-actions`, `/backend/b2b/settlements`, and `/backend/b2b/credentials`, a signed tenant-facing read-only operator portal is available at `/api/b2b/v1/portal`, JSON overview data is available at `/api/b2b/v1/portal/overview`, and web step-up confirmation is available. Expanded operator configuration, raw payload permissions UI, case management, and portal workflows still need implementation, but successful CLI and web credential, settlement, and manual wallet actions now leave unified operator audit events.
