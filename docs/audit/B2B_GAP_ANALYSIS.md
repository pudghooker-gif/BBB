...
- Dedicated B2B admin backoffice and operator portal remain incomplete. A read-only backend operations dashboard now boots at `/backend/b2b`, but web step-up forms for dangerous actions and a tenant-scoped operator portal still need implementation.
