...
- Dedicated B2B admin backoffice and full operator portal UX remain incomplete. A B2B-RBAC-protected backend operations dashboard now boots at `/backend/b2b`, step-up protected manual wallet and settlement workflow screens are available at `/backend/b2b/wallet/manual-actions` and `/backend/b2b/settlements`, a signed tenant-facing read-only operator portal is available at `/api/b2b/v1/portal`, JSON overview data is available at `/api/b2b/v1/portal/overview`, and web step-up confirmation is available. Expanded operator/credential web workflows, raw payload permissions UI, case management, and portal workflows still need implementation, but successful CLI-backed credential, settlement, and manual wallet actions now leave unified operator audit events.
