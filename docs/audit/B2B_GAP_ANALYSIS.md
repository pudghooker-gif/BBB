...
- Signed read-only operator portal endpoint at `/api/b2b/v1/portal`, JSON overview at `/api/b2b/v1/portal/overview`, and signed HTML workflow pages for credentials, games, sessions, transactions, settlements, cases, and docs, returning tenant-scoped profile, credential, game assignment, session, wallet, settlement, and reconciliation summaries without secrets or raw payloads.
