...
- Signed operator portal endpoint at `/api/b2b/v1/portal`, JSON overview at `/api/b2b/v1/portal/overview`, signed HTML workflow pages for credentials, games, sessions, transactions, settlements, cases, callbacks, reports, support, and docs, and a signed operator support-comment endpoint at `/api/b2b/v1/portal/support/cases/{transaction_uid}/comments`. Portal surfaces return tenant-scoped profile, credential, game assignment, session, wallet, callback, support, settlement, and reconciliation summaries without secrets or raw payloads; support comments are redacted before persistence and audited.
...
- Dedicated B2B admin backoffice and full operator portal UX remain incomplete. A B2B-RBAC-protected backend operations dashboard now boots at `/backend/b2b`, step-up protected manual wallet, settlement workflow, credential lifecycle, operator configuration, raw payload review, case-management, and audit trail screens are available at `/backend/b2b/wallet/manual-actions`, `/backend/b2b/settlements`, `/backend/b2b/credentials`, `/backend/b2b/operators`, `/backend/b2b/payloads`, `/backend/b2b/cases`, and `/backend/b2b/audit`, signed tenant-facing operator portal pages and support case comments are available under `/api/b2b/v1/portal/*`, JSON overview data is available at `/api/b2b/v1/portal/overview`, and web step-up confirmation is available. Portal production UX/staging validation and a richer operator support ticket lifecycle still need implementation, but successful CLI/web credential, settlement, manual wallet, operator configuration, raw payload review, case-management, and operator support-comment actions now leave unified operator audit events.
