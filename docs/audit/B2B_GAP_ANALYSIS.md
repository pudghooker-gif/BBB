...
- Signed read-only operator portal endpoint at `/api/b2b/v1/portal` plus JSON overview at `/api/b2b/v1/portal/overview`, returning tenant-scoped profile, credential, game assignment, session, wallet, settlement, and reconciliation summaries without secrets or raw payloads.
...
- B2B backend operations dashboard is wired into the existing authenticated backend at `/backend/b2b`; web step-up confirmation routes are available under `/backend/b2b/step-up/{action}`; a B2B-RBAC and web-step-up protected manual wallet action screen is available at `/backend/b2b/wallet/manual-actions`.
...
- Production-grade wallet state machine still needs external provider certification plus settlement approval screens wired through the web step-up middleware. The current foundation has explicit transition validation, append-only transition logging, recursive sensitive-field redaction, retryable `unknown`, retry-budget `dead_letter`, operator-scoped status lookup, provider-declared status/rollback contracts, reconciliation item scanning/reporting with conservative operator `transaction_status` resolution, bounded rollback recovery, privileged CLI guard, audited CLI and web manual actions, deterministic settlement exports, audited settlement submit/approve/reject commands, and session-bound web step-up confirmation.
- Dedicated B2B admin backoffice and full operator portal UX remain incomplete. A B2B-RBAC-protected backend operations dashboard now boots at `/backend/b2b`, a step-up protected manual wallet action screen is available at `/backend/b2b/wallet/manual-actions`, a signed tenant-facing read-only operator portal is available at `/api/b2b/v1/portal`, JSON overview data is available at `/api/b2b/v1/portal/overview`, and web step-up confirmation is available. Settlement mutation screens and expanded portal workflows still need implementation, but successful CLI-backed credential, settlement, and manual wallet actions now leave unified operator audit events.
