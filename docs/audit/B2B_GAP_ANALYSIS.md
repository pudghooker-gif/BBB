...
- Dedicated B2B admin backoffice and full operator portal UX remain incomplete. A B2B-RBAC-protected backend operations dashboard now boots at `/backend/b2b`, step-up protected manual wallet, settlement workflow, credential lifecycle, operator configuration, raw payload review, and case-management screens are available at `/backend/b2b/wallet/manual-actions`, `/backend/b2b/settlements`, `/backend/b2b/credentials`, `/backend/b2b/operators`, `/backend/b2b/payloads`, and `/backend/b2b/cases`, a signed tenant-facing read-only operator portal is available at `/api/b2b/v1/portal`, JSON overview data is available at `/api/b2b/v1/portal/overview`, and web step-up confirmation is available. Expanded operator portal workflows, including operator-visible case follow-up, still need implementation, but successful CLI/web credential, settlement, manual wallet, operator configuration, raw payload review, and case-management actions now leave unified operator audit events.
