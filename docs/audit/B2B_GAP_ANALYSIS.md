...
- Wallet transaction table, idempotency key, changed-payload conflict detection, callback attempt logging, append-only state transition log, status lookup endpoint, reconciliation item queue, bounded retry budget to `dead_letter`, retry command, and sandbox wallet.
...
- Feature tests now cover HMAC success/failure/replay, tenant isolation for sessions/reports/settlements/wallet attempts, request validation for launch and wallet payloads, wallet idempotency conflicts, wallet status transition logging, status lookup scoping, and reconciliation scanning.
...
- Production-grade wallet state machine still needs provider/operator status lookup contracts, manual review operations, reversal controls, settlement-grade reconciliation reports, and full rollback-required recovery flows. The current foundation has explicit transition validation, append-only transition logging, retryable `unknown`, retry-budget `dead_letter`, operator-scoped status lookup, and reconciliation item scanning.
