...
- Dedicated B2B admin backoffice and full operator portal UX remain incomplete. A B2B-RBAC-protected read-only backend operations dashboard now boots at `/backend/b2b`, a signed tenant-facing read-only operator portal is available at `/api/b2b/v1/portal`, JSON overview data is available at `/api/b2b/v1/portal/overview`, and web step-up confirmation is available. Dangerous mutation screens and expanded portal workflows still need implementation, but successful CLI-backed credential, settlement, and manual wallet actions now leave unified operator audit events.
