...
- Composer dependency audit now runs inside `b2b:release-check --production`. The PHP 7.4-compatible dependency refresh reduced findings from 39 advisories across 16 packages to 3 remaining Laravel framework advisories. Treat the remaining Laravel advisories, abandoned packages, and the PHP/Laravel major-upgrade plan as production blockers.
