...
The repository previously tracked sensitive/runtime artifacts such as `.env`, `.env_old`, WebSocket TLS key/cert files, a SQL dump, `vendor`, `composer.phar`, and `composer-setup.php`. They are removed from the current tree and covered by ignore/export-ignore rules; rotate any values committed to history before deploying.
...
git rm -r --cached vendor composer.phar composer-setup.php PTWebSocket/node_modules 2>/dev/null || true
git add .gitignore
