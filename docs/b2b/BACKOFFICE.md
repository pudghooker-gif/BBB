...
They are protected by the existing backend `auth`, `2fa`, `access.admin.panel`, `only_for_admin`, and dedicated `b2b.admin:*` controls. The dashboard does not expose API secrets, raw wallet payloads, callback bodies, player identifiers, or silent financial mutation controls. The default payload review table keeps request/response bodies redacted with the wallet redaction policy. Mutating wallet, settlement, credential, operator, and raw-payload access actions are kept on dedicated routes with session-bound web step-up middleware.
...
- links to manual wallet actions, settlement workflow, credential lifecycle, operator configuration, payload review, health, readiness, metrics, and the repository OpenAPI artifact path.
...
Richer case-management and expanded operator portal workflows still need dedicated web screens before this becomes a complete production backoffice.
